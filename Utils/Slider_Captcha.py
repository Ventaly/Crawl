...
import asyncio
import  os
from Utils.GetImage import get_image_src_and_download
import cv2 as cv
import random
...
async def solve_slider(page):
    ...
    MAX_RETRY = 10  # 最大重试次数
    retry_count = 0
    while not (os.path.exists(background_path) and os.path.exists(block_path)):
        print("等待图片下载中...")
        # 获取验证图片路径
        ...
        if retry_count > MAX_RETRY:
            print("等待超时，图片未下载成功")
            return  # 超时退出
        await asyncio.sleep(1)  # 每次等待 1 秒
    # 定位滑块元素
